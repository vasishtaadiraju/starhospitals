...
async function printDoctors(url, body) {
    ...
    doctors.forEach((result) => {
     ...
     <div class="doctors-card__rt">
         ...
         <div>
         <div target="_blank"  data-name="${
             result.doctor.name
         }" data-speciality="${formDetails.speciality}" data-branch="${
            formDetails.branch
        }" data-href="https://api.starhs.in/patient-portal/doctors/info/${
            branch_slug == "financial-district"
                ? `nanakramguda`
                : `${branch_slug}`
        }/${
         ...
         ${
             result.doctor.video_consultation == "yes"
                 ? `<a href="/doctors/book-a-video-consultation" class="doctors-card__rt__btn">
